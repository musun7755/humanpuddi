...
def _generate(slot: str) -> dict[str, str]:
    ...
    prompt = f"""系統於 {SLOTS[slot]} 觸發產文。這只是排程資訊，不是文案主題，也不限制赫湦此刻能想到的內容。

請為赫湦產生一則純文字 Threads 限時貼文候選。
...
- 繁體中文，15～180 字，可自然分行。
- 像當下碎念，不像正式貼文或社群小編。
- 自由選擇念頭、場景與事件；不要為了呼應排程，刻意提到中午、晚上、午餐、晚餐、睡前、光線或固定時段活動。
- 只有當時間本身恰好是這則內容不可取代的核心時，才自然提及時段；不得把排程時段當作發想起點。
