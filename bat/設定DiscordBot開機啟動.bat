...
set "BOT_SCRIPT=%~dp0..\src\discord_review_bot.py"
set "PYTHONW="
...
schtasks /Create /F /SC ONLOGON /TN "%TASK_NAME%" /TR "\"%PYTHONW%\" \"%BOT_SCRIPT%\" --scheduled" /RL LIMITED
if errorlevel 1 goto :task_error

echo 已設定在登入 Windows 時於背景啟動 Discord Bot。
